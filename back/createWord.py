...
guessPoints = [[[0, 0, 0] for i in range(11)] for j in range(30)]
...
def detect():   
    ...
    while not stop:
        ...
        # wholeNormPoints = normBodyPoints + bodyPoints
        ...
        if len(bodyPoints) == 11:     
            ...
            guessPoints.pop(-1)
            if (endMove >= 5) and guessPoints[MIN_FRAMES][10] != [0.0, 0.0, 0.0]:
                while guessPoints[-1][0] == [0, 0, 0]:
                    guessPoints.insert(0, bodyPoints)
                    guessPoints.pop(-1)
                print('END!!!!', endMove)
                word = 'start'
                ...
                f.write(json.dumps(guessPoints))
                f.close()
                exit()
